...
function App() {
  ...
  const searchTracks = async () => {
    ...
    try {
      ...
      setResults(data || []);
    } catch (err) { setError("Symphony offline"); }
  ...
  const selectTrack = async (track: any) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("500 Nodos bloqueados");
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#eee] font-sans overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto flex items-center gap-4">
          ...
          <div className="relative flex-1">
            <input 
              ...
              placeholder="Buscar en 500 fuentes..."
              value={query}
      ...
      <main className="max-w-6xl mx-auto p-4 pb-40">
        {error && (
          <div className="mb-8 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-red-500 flex items-center gap-3">
            <AlertCircle size={20} /> <span className="font-bold text-xs uppercase tracking-widest">{error}</span>
          </div>
        )}
        ...
        {loading && (
          <div className="flex flex-col items-center py-20">
            ...
            <p className="text-[10px] font-black tracking-[0.6em] text-red-600 mt-6 uppercase">Symphony Nitro: 500 Nodos</p>
          </div>
        )}
      ...
      {/* Mini & Full Player */}
      {currentTrack && (
        <div className={`fixed bottom-0 left-0 right-0 z-50 transition-all duration-700 cubic-bezier(0.19, 1, 0.22, 1) ${isPlayerOpen ? 'h-full bg-[#050505]' : 'h-24 bg-[#080808]/95 backdrop-blur-xl border-t border-white/5'}`}>
          <div className={`max-w-5xl mx-auto h-full flex flex-col px-6 ${isPlayerOpen ? 'pt-12' : 'justify-center'}`}>
            ...
            <div className={`flex items-center ${isPlayerOpen ? 'flex-col gap-10 flex-1 justify-center' : 'flex-row gap-4'}`}>
              <div className={`relative transition-all duration-700 ${isPlayerOpen ? 'w-80 h-80' : 'w-14 h-14'}`}>
                <img src={currentTrack.thumbnail} className={`w-full h-full object-cover shadow-2xl transition-all duration-700 ${isPlayerOpen ? 'rounded-[4rem]' : 'rounded-xl'}`} />
      ...
      <audio ref={audioRef} src={currentTrack?.streamUrl} onTimeUpdate={() => setProgress((audioRef.current!.currentTime / audioRef.current!.duration) * 100)} onEnded={() => setIsPlaying(false)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} />
    </div>
  );
}
